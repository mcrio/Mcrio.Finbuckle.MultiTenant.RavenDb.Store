    ...
    where TUniqueReservation : UniqueReservation
{
    ...
    public virtual async Task<bool> TryAddAsync(TTenantInfo tenantInfo)
    {
        ...
        {
            throw new Exception("Tenant identifier cannot be null or empty.");
        }
        ...
        Debug.Assert(
            ...
            "Tenant ID must be set at this point"
        );
    ...
    public virtual async Task<bool> TryUpdateAsync(TTenantInfo tenantInfo)
    {
        ...
        Debug.Assert(
            !string.IsNullOrWhiteSpace(tenantInfo.Id),
            "Expected Tenant ID to be already set."
        );
    ...
    public virtual async Task<bool> TryRemoveAsync(string tenantId)
    {
        ...
        TTenantInfo? entity = await Session.LoadAsync<TTenantInfo>(tenantId).ConfigureAwait(false);
        if (entity is null)
        {
            Logger.LogError(
        ...
        Debug.Assert(
            !string.IsNullOrWhiteSpace(entity.Identifier),
            "Expected Tenant Identifier to be set."
        );
    ...
    public virtual async Task<TTenantInfo?> TryGetByIdentifierAsync(string identifier)
    {
        ...
        {
            throw new ArgumentException(nameof(identifier) + " must not be empty.");
        }

        return await Session.Query<TTenantInfo>()
            .Where(entity => entity.Identifier != null && entity.Identifier.Equals(identifier))
    ...
    {
        return await Session.LoadAsync<TTenantInfo>(id).ConfigureAwait(false);
    }
    ...
    public virtual async Task<PaginatedResult<TTenantInfo>> GetAllPaginatedAsync(int page, int itemsPerPage)
    {
        ...
        return new PaginatedResult<TTenantInfo>((int)stats.TotalResults, result);
    }
